...
public class service4 extends Service {
    ...
    public static class service4_BR extends BroadcastReceiver {
        @Override // android.content.BroadcastReceiver
        public void onReceive(Context context, Intent intent) {
            BA.LogInfo("** Receiver (service4) OnReceive **");
            Intent intent2 = new Intent(context, (Class<?>) service4.class);
            if (intent != null) {
    ...
    public void onCreate() {
        ...
        mostCurrent = this;
        if (processBA == null) {
            BA BA = new BA(this, (BALayout) null, (BA) null, "com.com.androidrr", "com.com.androidrr.service4");
            processBA = BA;
        ...
        if (BA.isShellModeRuntimeCheck(processBA)) {
            BA BA2 = processBA;
            BA2.raiseEvent2(null, true, "CREATE", true, "com.com.androidrr.service4", BA2, this._service, Float.valueOf(Common.Density));
        }
        if (ServiceHelper.StarterHelper.startFromServiceCreate(processBA, false)) {
            processBA.setActivityPaused(false);
            BA.LogInfo("*** Service (service4) Create ***");
    ...
    public int onStartCommand(final Intent intent, int i, int i2) {
        if (!ServiceHelper.StarterHelper.onStartCommand(processBA, new Runnable() { // from class: com.com.androidrr.service4.1
            @Override // java.lang.Runnable
            public void run() {
                service4.this.handleStart(intent);
            }
        })) {
            ServiceHelper.StarterHelper.addWaitForLayout(new Runnable() { // from class: com.com.androidrr.service4.2
                @Override // java.lang.Runnable
                public void run() {
                    service4.processBA.setActivityPaused(false);
                    BA.LogInfo("** Service (service4) Create **");
                    service4.processBA.raiseEvent(null, "service_create", new Object[0]);
                    service4.this.handleStart(intent);
    ...
    public void handleStart(Intent intent) {
        BA.LogInfo("** Service (service4) Start **");
    ...
    public void onTimeout(int i) {
        BA.LogInfo("** Service (service4) Timeout **");
        Map map = new Map();
        map.Initialize();
    ...
    public void onDestroy() {
        super.onDestroy();
        BA.LogInfo("** Service (service4) Destroy **");
